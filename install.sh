...
printf '\n'
printf '  Cursor MCP (~/.cursor/mcp.json) — use absolute paths:\n'
printf '    command: %s/keel\n' "$INSTALL_DIR"
printf '    args: ["mcp"]\n'
printf '    env.KEEL_INDEX_DB: /absolute/path/to/project/.keel/index.db\n'
printf '  (Prefer KEEL_INDEX_DB over cwd; see README Quick start.)\n'
printf '\n'
printf '  keel definition SomeSymbol # CLI also works; auto-indexes if needed\n'
